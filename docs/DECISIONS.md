...
| D12 | **Moke is drawn in an anime cel style, built in code** | Owner request with a reference picture (2026-09-24): white fur with lavender cel shading, ink outlines, soft fur tufts, big glossy brown eyes, blush, "w" smile. `ToonMokeVisual` replaces the old placeholder. It refines D5 and keeps D7: gameplay never touches it, and a `moke.glb` could still replace it via `createMokeVisual()`. | 2026-09-24 |
