...
| 10 | Polish: movement, camera, visuals, audio, UI, performance | **In progress** (started 2026-09-24 at the owner's request). Done, awaiting the owner's look: Moke restyled after the real dog (`ToonMokeVisual`: curly fur, soft shading, realistic dark eyes, button nose, blinks, shorter ears, blue collar with a "Moke" bone tag); title screen fits without scrolling and the logo "O" sits in line; a trick button (Q / X: belly up, beg, give paw, spin), with sniff moved to R / right stick press. Remaining candidates: `docs/CURRENT_STATE.md` → Known Issues. |
