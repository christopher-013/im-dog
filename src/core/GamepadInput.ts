...
export function applyStickDeadzone(x: number, y: number, deadzone = GAMEPAD.deadzone, out: Vec2Like = { x: 0, y: 0 }): Vec2Like {
...
export class GamepadInput {
  ...
  update(gamepads: readonly (GamepadLike | null)[], input: InputState, dt: number): void {
    ...
    const stick = applyStickDeadzone(pad.axes[0] ?? 0, pad.axes[1] ?? 0, GAMEPAD.deadzone, this.stick);
