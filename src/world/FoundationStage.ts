...
import { HOUSE_SCALE, MOKE_SIZE } from '../config/world';
...
export class FoundationStage {
  ...
  /** Where Moke will stand, at eye height. The preview camera orbits this point. */
  readonly spawnPoint = new Vector3(0.3, MOKE_SIZE.eyeHeight, 0.55);
  ...
  private buildShell(m: Record<'floor' | 'wall' | 'accentWall' | 'ceiling' | 'trim', Material>): void {
    ...
    this.add(new BoxGeometry(W, 0.1, D), m.floor, [0, -0.05, 0], { cast: false });
    this.add(new BoxGeometry(W + 2 * T, 0.1, D + 2 * T), m.ceiling, [0, H + 0.05, 0], { receive: false });

    this.add(new BoxGeometry(W + 2 * T, H, T), m.accentWall, [0, H / 2, -halfD - T / 2]);
    this.add(new BoxGeometry(W + 2 * T, H, T), m.wall, [0, H / 2, halfD + T / 2]);
    this.add(new BoxGeometry(T, H, D), m.wall, [halfW + T / 2, H / 2, 0]);

    // Left wall, built around the window opening so the sun only gets in through the glass.
    const x = -halfW - T / 2;
    const { zMin, zMax, yMin, yMax } = WINDOW;
    const midY = (yMin + yMax) / 2;
    this.add(new BoxGeometry(T, yMin, D), m.wall, [x, yMin / 2, 0]);
    this.add(new BoxGeometry(T, H - yMax, D), m.wall, [x, (H + yMax) / 2, 0]);
    this.add(new BoxGeometry(T, yMax - yMin, zMin + halfD), m.wall, [x, midY, (zMin - halfD) / 2]);
    this.add(new BoxGeometry(T, yMax - yMin, halfD - zMax), m.wall, [x, midY, (zMax + halfD) / 2]);
  ...
  private buildFurniture(m: Record<'rug' | 'couch' | 'pillowLeaf' | 'pillowMustard' | 'walnut', Material>): void {
    ...
    this.add(new RoundedBoxGeometry(couchW - 0.1, seatH, couchD, 3, 0.07), m.couch, [cx, seatH / 2, cz]);
    this.add(new RoundedBoxGeometry(couchW - 0.1, backH - seatH + 0.05, 0.28, 3, 0.09), m.couch, [
      cx,
      (seatH - 0.05 + backH) / 2,
      backZ + 0.13,
    ]);
    const armGeometry = new RoundedBoxGeometry(0.24, 0.64, couchD + 0.02, 3, 0.09);
    this.add(armGeometry, m.couch, [cx - couchW / 2 + 0.12, 0.32, cz]);
    this.add(armGeometry, m.couch, [cx + couchW / 2 - 0.12, 0.32, cz]);
    ...
    // Coffee table: tall enough for Moke to walk underneath.
    const tableH = HOUSE_SCALE.coffeeTableHeight;
    const top = 0.05;
    const tx = 0.3;
    const tz = -1.15;
    this.add(new RoundedBoxGeometry(1.15, top, 0.62, 2, 0.02), m.walnut, [tx, tableH - top / 2, tz]);
    ...
    for (const [dx, dz] of [
      ...
      this.add(legGeometry, m.walnut, [tx + dx, (tableH - top) / 2, tz + dz]);
    }
  }
